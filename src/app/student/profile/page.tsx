...
import { User, Loader2, Eye, EyeOff, ArrowLeft, AlertCircle, Info } from 'lucide-react';
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (student && user) {
      ...
      try {
        ...
        setStudent(updatedStudentResult);
        
        if (newPassword || (user.email !== updatedStudentResult.email)) {
            await addCredential({
                userId: user.id,
