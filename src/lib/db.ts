...
function setupDb() {
  ...
  const insertLeaveRequest = db.prepare('INSERT INTO leave_requests (requesterId, requesterName, requesterRole, startDate, endDate, reason, status) VALUES (?, ?, ?, ?, ?, ?, ?)');
  ...
  db.transaction(() => {
    ...
    leaveRequests.forEach(lr => insertLeaveRequest.run(lr.requesterId, lr.requesterName, lr.requesterRole, lr.startDate, lr.endDate, lr.reason, lr.status));
